...
class RomanNumeralGeneratorTest extends \PHPUnit_Framework_TestCase
{
  ...
  /**
   * Tests roman numeral uppercase generation.
   *
   * @dataProvider providerTestGeneration
   */
  public function testGenerationForUpperCaseStringResult($number, $expected)
  {
    $this->assertEquals($expected, $this->generator->generate($number));
  }

  /**
   * Tests roman numeral lowercase generation.
   *
   * @dataProvider providerTestGeneration
   */
  public function testGenerationForLowerCaseStringResult($number, $expected)
  {
